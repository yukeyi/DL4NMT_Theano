...
class NMTModel(object):
    ...
    def __init__(self, options, given_params=None):
        ...
        if 'dropout_out' not in options:
            self.O['dropout_out'] = 0.5 if 'fix_dp_bug' not in options or not options['fix_dp_bug'] else self.O[
                'use_dropout']  # recover previous dropout_before_softmax
    ...
    def build_model(self, confidence, candidate_size, set_instance_variables=False):
        ...
        if self.O['plot_graph'] is not None:
            ...
            theano.printing.pydotprint(
                cost_F,
                outfile='pictures/F/pre_compile_{}'.format(self.O['plot_graph']),
        ...
        if self.O['plot_graph'] is not None:
            ...
            theano.printing.pydotprint(
                cost_CE,
                outfile='pictures/CE/pre_compile_{}'.format(self.O['plot_graph']),
                var_with_name_simple=True,
            )
            print('Done')
        if self.O['plot_graph'] is not None:
            ...
            theano.printing.pydotprint(
                cost_M,
                outfile='pictures/M/pre_compile_{}'.format(self.O['plot_graph']),
                var_with_name_simple=True,
            )
            print('Done')
        if self.O['plot_graph'] is not None:
            ...
            theano.printing.pydotprint(
                cost_B,
                outfile='pictures/B/pre_compile_{}'.format(self.O['plot_graph']),
                var_with_name_simple=True,
            )
            print('Done')
    ...
    def build_sampler(self, **kwargs):
        ...
        self.f_next = [theano.function(inps, outs, name='f_next', profile=profile), f_att_projected]
        print('Done')

        return self.f_init, self.f_next
    ...
    def gen_batch_sample(self, f_init, f_next, x, x_mask, trng=None, k=1, maxlen=30, eos_id=0, attn_src=False,
                         **kwargs):
        """
    ...
    def gen_batch_sample2(self, x, x_mask, k=1, maxlen=30, eos_id=0, **kwargs):
        ...
        ctx = np.repeat(ctx0, lives_k, axis=1)
        projected_context_ = self.f_next[1](ctx)
    ...
    def build_cost_Focal(self, y, y_mask, probs):
        ...
        y_flat = y.flatten()
        y_flat_idx = T.arange(y_flat.shape[0]) * self.O['n_words'] + y_flat
        cost = -(1 - probs.flatten()[y_flat_idx]) * T.log(probs.flatten()[y_flat_idx])
        cost = cost.reshape([y.shape[0], y.shape[1]])
        cost = (cost * y_mask).sum(0)
    ...
    def attention_projected_context(self, context, prefix='lstm', **kwargs):
        attention_layer_id = self.O['attention_layer_id']
        pre_projected_context_ = T.dot(context, self.P[_p(prefix, 'Wc_att', attention_layer_id)]) + self.P[
            _p(prefix, 'b_att', attention_layer_id)]
        return pre_projected_context_


class TModel(object):
    def __init__(self, n_input, n_hidden, n_output, optimizer = 'moment'):
        self.n_input = n_input
        self.n_hidden = n_hidden
        self.n_output = n_output
        self.state_var = T.matrix('states')
        self.action_var = T.ivector('actions')
        self.reward_var = T.vector('reward')
        self.network = self.build_model(n_input, n_hidden, n_output, self.state_var)
        ...
        if(optimizer == 'adam'):
            self.updates = lasagne.updates.adam(self.loss, self.params, learning_rate=0.01)
        elif(optimizer == 'sgd'):
            self.updates = lasagne.updates.sgd(self.loss, self.params, learning_rate=0.01)
        ...
        self.train_fn = theano.function([self.state_var, self.action_var, self.reward_var], self.loss, updates=self.updates)

        self.pre_fn = theano.function([self.state_var], [self.prediction])

    def build_model(self, n_input, n_hidden, n_output, state_var=None, depth=2, drop_input=.2,
                    drop_hidden=.5):
        # Input layer and dropout (with shortcut `dropout` for `DropoutLayer`):
        network = lasagne.layers.InputLayer(shape=(None, n_input),
                                            input_var=state_var)
        if drop_input:
        ...
        network = lasagne.layers.DenseLayer(network, n_output, nonlinearity=softmax)
        return network

    def compute_loss(self):
        # self.loss_flatten = lasagne.objectives.categorical_crossentropy(self.prediction, self.action_var)
        # self.loss = self.loss_flatten.mean()
        one_hot_action = T.extra_ops.to_one_hot(self.action_var, self.n_output)
        neg_log_prob = T.mean(-T.log(self.prediction) * one_hot_action,axis=1)
        #print(neg_log_prob)
        cost = T.mean(self.reward_var * neg_log_prob)

        return cost

    def predict(self, x):
        action = self.pre_fn(x)
        return action
    ...
    def train(self, data):
        ...
        for item in data:
            ...
            reward = reward + [item['reward']]*len(item['action'])
        cost = self.train_fn(state, action, reward)
        return cost

    def save_policy(self, epoch, filename=None):
        if(filename == None):
            print("No filename, not saving")
        np.savez(filename + str(epoch), *[parameter.get_value() for parameter in self.params])
        return
    ...
    def load_policy(self, filename=None):
        if(filename == None):
            ...
            print("Policy file not exist, do not load")
            return
        else:
