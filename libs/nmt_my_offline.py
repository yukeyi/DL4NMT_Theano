...
def initial_dataseed(num, step):
    numlist = []
    for i in range(num / step):
        numlist.append(step*i+np.random.randint(step))
    return numlist

def train(teacher = None,
          update_frequence_batches = 1000,
          state_batchsize = 32,
          candidate_size= 8,
...
'''.format(
    ...
    seed = initial_dataseed(4 * batch_size, 4)
    ...
    # Build model
    trng, use_noise, \
    x, x_mask, y, y_mask, y_hat_reward, predicted_y, predicted_y_mask,\
    opt_ret, \
    cost_CE_flatten, test_cost_CE, \
    cost_F_flatten, test_cost_F, \
    cost_M_flatten, test_cost_M, \
    cost_B_flatten, test_cost_B, \
    cost_MBO_flatten, test_cost_MBO, \
    cost_RISK_flatten, test_cost_RISK, predicted_rewards,\
    x_emb, probs, alpha = model.build_model(0.5, candidate_size)

    inps = [x, x_mask, y, y_mask]
    inps_B = [x, x_mask, y, y_mask, y_hat_reward]
    inps_M = [x, x_mask, y, y_mask, predicted_y, predicted_y_mask, alpha]
    inps_MBO = [x, x_mask, y, y_mask, predicted_y, predicted_y_mask]
    inps_RISK = [x, x_mask, y, y_mask, predicted_y, predicted_y_mask, predicted_rewards]
    ...
    test_cost_MBO = test_cost_MBO.mean() #FIXME: do not regularize test_cost here
    cost_MBO = cost_MBO_flatten.mean()
    cost_MBO = l2_regularization(cost_MBO, model.P, decay_c)
    cost_MBO = regularize_alpha_weights(cost_MBO, alpha_c, model_options, x_mask, y_mask, opt_ret)
    test_cost_RISK = test_cost_RISK.mean() #FIXME: do not regularize test_cost here
    cost_RISK = cost_RISK_flatten.mean()
    cost_RISK = l2_regularization(cost_RISK, model.P, decay_c)
    cost_RISK = regularize_alpha_weights(cost_RISK, alpha_c, model_options, x_mask, y_mask, opt_ret)

    print 'Building f_cost...',
    f_cost_CE = theano.function(inps, test_cost_CE, profile=profile)
    f_cost_F = theano.function(inps, test_cost_F, profile=profile)
    f_cost_M = theano.function(inps_M, test_cost_M, profile=profile,on_unused_input='warn')
    f_cost_B = theano.function(inps_B, test_cost_B, profile=profile)
    f_cost_MBO = theano.function(inps_MBO, test_cost_MBO, profile=profile, on_unused_input='warn')
    f_cost_RISK = theano.function(inps_RISK, test_cost_RISK, profile=profile, on_unused_input='warn')
    ...
    #f_cost_flatten_M = theano.function(inps_M, cost_M_flatten, profile=profile,on_unused_input='warn')
    ...
    grads_MBO = tensor.grad(cost_MBO, wrt=itemlist(model.P))
    grads_RISK = tensor.grad(cost_RISK, wrt=itemlist(model.P))
    ...
    if dist_type != 'mpi_reduce': #build grads clip into computational graph
        ...
        grads_MBO, g2_MBO = clip_grad_remove_nan(grads_MBO, clip_shared, model.P)
        grads_RISK, g2_RISK = clip_grad_remove_nan(grads_RISK, clip_shared, model.P)
    ...
    f_grad_shared_MBO, f_update_MBO, grads_shared_MBO, imm_shared_MBO = Optimizers[optimizer](
        lr, model.P, grads_MBO, inps_MBO, cost_MBO, g2=g2_MBO, given_imm_data=given_imm_data, alpha = ada_alpha)
    f_grad_shared_RISK, f_update_RISK, grads_shared_RISK, imm_shared_RISK = Optimizers[optimizer](
        lr, model.P, grads_RISK, inps_RISK, cost_RISK, g2=g2_RISK, given_imm_data=given_imm_data, alpha = ada_alpha)
    print 'Done'
    ...
    if dist_type == 'mpi_reduce':
        ...
        f_grads_clip_MBO = make_grads_clip_func(grads_shared=grads_shared_MBO, mt_tparams=model.P, clip_c_shared=clip_shared)
        f_grads_clip_RISK = make_grads_clip_func(grads_shared=grads_shared_RISK, mt_tparams=model.P, clip_c_shared=clip_shared)
    ...
    if dist_type == 'mv':
        ...
        rec_grads_MBO = [np.zeros_like(p.get_value()) for p in model.P.itervalues()]
        rec_grads_RISK = [np.zeros_like(p.get_value()) for p in model.P.itervalues()]
        ...
        for i in range(0, validFreq):
            ...
            if(loss_name == 'MBO'):
                ...
                for i in range(0,len(predicted_y)):
                    if(predicted_y[i] == []):
                        if(i % candidate_size == 0):
                            predicted_y[i] = predicted_y[i+1]
                        else:
                            predicted_y[i] = predicted_y[i-1]
                rewards = []
                for i in range(0,len(predicted_y)):
                    rewards.append(_bleu(y[i/candidate_size],predicted_y[i])[0][:, -1][-1])
                    if(i % candidate_size == (candidate_size-1)):
                        max_index = np.array(rewards).argmax()
                        temp = predicted_y[i-i%candidate_size]
                        predicted_y[i - i % candidate_size] = predicted_y[i-i%candidate_size+max_index]
                        predicted_y[i - i % candidate_size + max_index] = temp
                        rewards = []
                ...
                predicted_rewards = []
                for i in range(0,len(predicted_y)):
                    predicted_rewards.append(_bleu(y[i/candidate_size],predicted_y[i])[0][:, -1][-1])
                predicted_rewards = np.array(predicted_rewards)
                #predicted_rewards -= predicted_rewards.mean()
                original_x = copy.deepcopy(x)
                x, x_mask, y, y_mask = prepare_data(original_x, y, maxlen=maxlen)
                _, _, predicted_y, predicted_y_mask = prepare_data(np.array(original_x).repeat([candidate_size],axis = 0), predicted_y, maxlen=sys.maxint)
            elif(loss_name == 'B'):
            ...
            if(loss_name == 'CE'):
                ...
                # compute cost, grads
                if dist_type != 'mpi_reduce':
                    cost_M, g2_value_M = f_grad_shared_M(x, x_mask, y, y_mask, predicted_y, predicted_y_mask, r_value)
                else:
                    cost_M = f_grad_shared_M(x, x_mask, y, y_mask, predicted_y, predicted_y_mask, r_value)

                #if np.mod(uidx, saveFreq) == 0:
                #    print(f_cost_flatten_M(x, x_mask, y, y_mask, predicted_y, predicted_y_mask, r_value))
                ...
                # compute cost, grads
                if dist_type != 'mpi_reduce':
                    cost_MBO, g2_value_MBO = f_grad_shared_MBO(x, x_mask, y, y_mask, predicted_y, predicted_y_mask)
                else:
                    cost_MBO = f_grad_shared_MBO(x, x_mask, y, y_mask, predicted_y, predicted_y_mask)
                ...
                if np.isnan(cost_MBO) or np.isinf(cost_MBO):
                    message('NaN detected')
                    sys.stdout.flush()
                    clip_shared.set_value(np.float32(clip_shared.get_value() * 0.8))
                    message('Discount clip value to {} at iteration {}'.format(clip_shared.get_value(), uidx))
                ...
                if np.mod(uidx, dispFreq) == 0:
                    message('Worker {} Epoch {} Update {} Cost {:.5f} G2 {:.5f} UD {:.5f} Time {:.5f} s'.format(
                        worker_id, eidx, uidx, float(cost_MBO), float(g2_value_MBO), ud, time.time() - start_time,
                    ))
                    sys.stdout.flush()
                ...
                if dist_type != 'mpi_reduce':
                    ...
                    cost_RISK = f_grad_shared_RISK(x, x_mask, y, y_mask, predicted_y, predicted_y_mask)
                ...
                if np.isnan(cost_RISK) or np.isinf(cost_RISK):
                    message('NaN detected')
                    sys.stdout.flush()
                    clip_shared.set_value(np.float32(clip_shared.get_value() * 0.8))
                    message('Discount clip value to {} at iteration {}'.format(clip_shared.get_value(), uidx))
                ...
                if np.mod(uidx, dispFreq) == 0:
                    message('Worker {} Epoch {} Update {} Cost {:.5f} G2 {:.5f} UD {:.5f} Time {:.5f} s'.format(
                        worker_id, eidx, uidx, float(cost_RISK), float(g2_value_RISK), ud, time.time() - start_time,
                    ))
                    sys.stdout.flush()
            ...
            if np.mod(uidx, validFreq) == 0:
                ...
                # r_value = -60 + 2*valid_bleu
                message('Worker {} Valid cost {:.5f} Small train cost {:.5f} Valid BLEU {:.2f} Bad count {} R_value{}'.format(
                    worker_id, valid_cost, small_train_cost, valid_bleu, bad_counter, r_value))
                sys.stdout.flush()
